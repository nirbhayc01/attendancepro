    ...
    activeDays.forEach(day => {
        ...
        let slotsHTML = slots.map((s, i) => `
            <div class="ob-slot-row">
                <input type="text" placeholder="Tap to set time" value="${s[0]}" readonly onclick="openTimePicker('onboarding', ${i}, '${day}', this.value)" style="cursor:pointer; font-size:13px;">
                <select onchange="updateObSlot('${day}',${i},1,this.value)" style="margin-bottom:0;">
                    ${obSubjects.map(sub => `<option ${s[1]===sub?'selected':''}>${sub}</option>`).join('')}
                </select>
...
function loadLectures(){
  ...
  if (activeHoliday) {
      ...
      // --- 3. HOLIDAY / FESTIVAL CARD (Cozy Caramel Ceramic Cup) ---
      else {
          ...
          svgIcon = `
            <svg viewBox="0 0 100 100" style="width: 80px; height: 80px; margin-bottom: 16px; overflow: visible;">
                <defs>
                    <radialGradient id="cup3D" cx="30%" cy="30%" r="70%">
                        <stop offset="0%" stop-color="#faedcd"/> <stop offset="100%" stop-color="#d4a373"/> </radialGradient>
                    <linearGradient id="saucer3D" x1="0%" y1="0%" x2="0%" y2="100%">
                        <stop offset="0%" stop-color="#523214"/>
                        <stop offset="100%" stop-color="#29190a"/>
                    </linearGradient>
                    <radialGradient id="liquid3D" cx="50%" cy="50%" r="50%">
                        <stop offset="0%" stop-color="#3e1f0b"/>
                        <stop offset="100%" stop-color="#1a0b02"/>
                    </radialGradient>
                </defs>
                <style>
                ...
                <circle cx="50" cy="55" r="35" fill="rgba(212, 163, 115, 0.15)"/>
                
                <g class="anim-float">
                    <ellipse cx="50" cy="82" rx="26" ry="6" fill="rgba(0,0,0,0.2)"/>
                    
                    <ellipse cx="50" cy="78" rx="30" ry="9" fill="url(#saucer3D)"/>
                    <ellipse cx="50" cy="77" rx="24" ry="7" fill="#3b230e"/>
                    
                    <path d="M 68 45 C 90 40, 90 70, 65 72" fill="none" stroke="url(#cup3D)" stroke-width="8" stroke-linecap="round"/>
                    <path d="M 68 45 C 88 42, 88 68, 65 72" fill="none" stroke="#bc8a5f" stroke-width="2" stroke-linecap="round" opacity="0.4"/>
                    
                    <path d="M 26 35 L 74 35 C 74 75, 66 82, 50 82 C 34 82, 26 75, 26 35 Z" fill="url(#cup3D)"/>
                    
                    <ellipse cx="50" cy="35" rx="24" ry="7" fill="#3e2723"/>
                    <ellipse cx="50" cy="35" rx="24" ry="7" fill="none" stroke="#fefae0" stroke-width="1.5" opacity="0.8"/>
                    
                    <ellipse cx="50" cy="36" rx="21" ry="5.5" fill="url(#liquid3D)"/>
                    <ellipse cx="58" cy="35" rx="6" ry="1.5" fill="#ffffff" opacity="0.2" transform="rotate(-15 58 35)"/>
                    <ellipse cx="40" cy="37" rx="3" ry="1" fill="#ffffff" opacity="0.1"/>
                    
                    <path d="M 40 28 C 35 15, 48 10, 42 -2" fill="none" stroke="#ffffff" stroke-width="3.5" stroke-linecap="round" opacity="0.35"/>
                    <path d="M 52 28 C 45 12, 60 8, 52 -5" fill="none" stroke="#ffffff" stroke-width="4.5" stroke-linecap="round" opacity="0.45"/>
                    <path d="M 62 26 C 58 15, 68 12, 64 0" fill="none" stroke="#ffffff" stroke-width="2.5" stroke-linecap="round" opacity="0.25"/>
                </g>
            </svg>`;
      }

      container.innerHTML = `
          <div class="holiday-card" style="border-color: ${colorTheme};">
              ${svgIcon}
...
function renderEditRows() { 
    ...
    lectures.forEach((lec, index) => { 
        ...
        container.innerHTML += `<div class="edit-row" style="display:flex; gap:8px; margin-bottom:12px; align-items:center;">
           <input type="text" class="edit-input" style="width:30%; margin-bottom:0; font-size:13px; padding: 10px 8px; cursor: pointer;" value="${lec[0]}" id="time_${index}" placeholder="Tap to set time" readonly onclick="openTimePicker('settings', ${index}, null, this.value)">
            <select class="edit-input" style="width:50%; margin-bottom:0; font-size:13px; padding: 10px 8px;" id="sub_${index}">
                ${optionsHTML}
            </select>
            <input type="number" class="edit-input" style="width:48px; margin-bottom:0; font-size:13px; padding:10px 8px; text-align:center;" value="${weight}" id="wt_${index}" min="1" max="10" title="Credit weight" placeholder="Cr">
            <button style="background:var(--red-glow); color:var(--red); border:none; border-radius:12px; padding:0; width:40px; height:46px; display:flex; align-items:center; justify-content:center; flex-shrink:0; transition:0.2s;" onclick="delEditRow(${index})">${ICONS.trash}</button>
        </div>`; 
    }); 
}
function addEditRow() { 
...
function saveNewTimetable() { const selectedDay = document.getElementById("editDaySelect").value; const container = document.getElementById("editContainer"); const rows = container.querySelectorAll(".edit-row"); const newDaySchedule = []; rows.forEach((row, index) => { const t = document.getElementById(`time_${index}`).value; const s = document.getElementById(`sub_${index}`).value.trim(); const w = parseInt(document.getElementById(`wt_${index}`)?.value) || 1; if(t && s) newDaySchedule.push([t, s, w > 1 ? w : undefined].filter(x => x !== undefined)); }); timetable[selectedDay] = newDaySchedule; localStorage.setItem("custom_timetable", JSON.stringify(timetable)); showToast(`Saved ${selectedDay}`); setTimeout(() => location.reload(), 500); }
